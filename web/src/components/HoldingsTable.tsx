  ...
  return (
    <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-panel" aria-labelledby="holdings-title">
      <div className="flex items-center justify-between border-b border-slate-100 px-6 py-5">
        <div>
          <h2 id="holdings-title" className="text-lg font-semibold text-ink">Holdings</h2>
          <p className="mt-1 text-xs text-slate-500">Live prices and portfolio performance</p>
        </div>
        ...
        <table className="w-full min-w-[1320px] text-left">
          <thead className="bg-slate-50 text-[11px] font-semibold uppercase tracking-wide text-slate-500">
            <tr>
              ...
              <th className="px-4 py-3 text-right">FIFO avg. price</th>
              <th className="px-4 py-3 text-right">LTP</th>
              <th className="px-4 py-3 text-right">Open cost</th>
              <th className="px-4 py-3 text-right">Current value</th>
              <th className="px-4 py-3 text-right">Total G/L</th>
              <th className="px-4 py-3 text-right">Day's G/L</th>
              <th className="px-4 py-3 text-right">Latest buy</th>
              <th className="px-6 py-3 text-right">Latest sell</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            {holdings.map((holding) => (
              <tr key={`${holding.exchange}:${holding.symbol}`} className="transition hover:bg-slate-50">
                <td className="px-6 py-4">
                  <Link className="font-semibold text-brand-700 hover:text-brand-600 hover:underline" to={`/stock/${holding.exchange}/${encodeURIComponent(holding.symbol)}`}>
                    {holding.companyName}
                  </Link>
                  <div className="mt-1 flex items-center gap-2 text-xs text-slate-500">
                    <span>{holding.symbol} · {holding.exchange}</span>
                    {holding.quoteStatus !== "live" ? (
                      <span className={`rounded px-1.5 py-0.5 font-semibold ${holding.quoteStatus === "unavailable" ? "bg-red-50 text-loss" : "bg-amber-50 text-amber-700"}`}>
                        {holding.quoteStatus}
                      </span>
                    ) : null}
                  </div>
...
function TradeButton({ label, className, onClick }: { label: string; className: string; onClick(): void }): JSX.Element {
  return (
    <button
      type="button"
      className={`rounded-md border px-2.5 py-1 text-[11px] font-bold ${className}`}
      onClick={onClick}
    >
      {label}
    </button>
...
function PnlCell({ money, percent }: { money: string | null; percent: string | null }): JSX.Element {
  ...
  return (
    <td className={`px-4 py-4 text-right text-sm font-semibold tabular-nums ${color}`}>
      <span className="block">{formatCurrency(money)}</span>
      <span className="mt-1 block text-[11px]">{formatPercent(percent, true)}</span>
    </td>
  );
}
